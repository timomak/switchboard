...
func testAccountStatus() {
    ...
    assertEqual(accountsSummaryLine(s!), "Claude: toptal", "summary line")
    ...
    assertEqual(accountsSummaryLine(linux!), "", "CLI login does not appear as a desktop selector")
    ...
    assertEqual(accountsSummaryLine(orphan!), "", "CLI status is retained only for usage discovery")
    ...
    let both = parseAccountStatus(Data(#"{"desktop":{"profiles":[{"label":"personal"}],"active_label":"personal"},"codex":{"available":true,"has_login":true,"active_label":"work","profiles":[{"label":"work"},{"label":"personal"}],"recovery_required":true}}"#.utf8))!
    assertEqual(both.codexAvailable, true, "Codex backend is available")
    assertEqual(both.codexActive, "work", "Codex active profile is independent")
    assertEqual(both.codexLabels, ["work", "personal"], "Codex profiles parsed")
    assertEqual(both.codexHasLogin, true, "live login can be saved")
    assertEqual(both.codexRecoveryRequired, true, "interrupted switch is surfaced")
    assertEqual(accountsSummaryLine(both), "Claude: personal   ·   Codex: work", "independent desktop summary")
    assertEqual(bare?.codexAvailable, false, "older backend hides unavailable Codex menu")
    let codexError = parseAccountStatus(Data(#"{"codex":{"available":true,"error":"Read failed"}}"#.utf8))
    assertEqual(codexError?.codexError, "Read failed", "Codex failure remains visible")
    assertEqual(codexSwitchArgs(label: "work"), ["codex-account", "switch", "work", "--yes"], "Codex switch cannot dispatch Claude account command")
    ...
    assertEqual(conflictPreview(many).hasSuffix("… and 2"), true, "preview counts the rest")
...
func testSwitchboardRequests() {
    let errors = parseAccountStatus(Data(#"{"codex":{"profiles":[{"label":"Personal"},{"label":"Work"}],"profile_errors":[{"label":"Broken","error":"Restore metadata"}]},"codex_cli":{"profile_errors":[{"label":"Broken CLI","error":"Restore metadata"}]},"claude_connection":{"state":"invalid","error":"Restore connection"},"codex_cloud":{"selection":{"error":"Field model changed","recovery_required":true}}}"#.utf8))!
    assertEqual(errors.codexLabels.count, 2, "invalid profile retains valid account choices")
    assertNil(errors.codexError, "isolated profile warning does not disable valid accounts")
    assertEqual(errors.codexProfileProblems, ["Broken: Restore metadata"], "profile warning retained")
    assertEqual(errors.codexCLIProfileProblems, ["Broken CLI: Restore metadata"], "CLI profile warning retained")
    assertEqual(errors.claudeConnectionError, "Restore connection", "Claude invalid status is visible")
    assertEqual(errors.cloudError, "Field model changed", "safe conflict message reaches settings")

    let data = Data(#"{"cli":{"active_label":"cli-personal","has_identity":true,"accounts":[]},"desktop":{"active_label":"desktop-work","profiles":[]},"codex":{"active_label":"codex-work"},"codex_cli":{"mode":"separate","active_label":"codex-personal","home":"/fixture/cli-home","profiles":[{"label":"codex-personal"}]}}"#.utf8)
    let cloud = Data(#"{"codex_cloud":{"profiles":[{"label":"azure-work","provider":"Azure","model":null}]}}"#.utf8)
    assertEqual(parseAccountStatus(cloud)?.cloudProfiles.first?.provider, "Azure", "connection metadata")
    assertEqual(boardUsageRequests(parseAccountStatus(cloud)!).count, 0, "saved connections do not fetch usage")
    let status = parseAccountStatus(data)!
    let requests = boardUsageRequests(status)
    assertEqual(requests.count, 4, "four independently bound usage targets")
    assertEqual(requests.first?.args.last, "--desktop", "Claude Desktop uses its OAuth client")
    assertEqual(requests.last?.home, "/fixture/cli-home", "independent Codex request uses CLI home")
    assertEqual(requests.last?.key, "codex:cli:codex-personal", "usage keyed by surface and account")
    var selected = status; selected.cloudActive = "team-connection"
    assertEqual(boardUsageRequests(selected).contains { $0.key.hasPrefix("codex:desktop:") }, false, "selected API connection never displays subscription quota")
    var shared = status; shared.codexCLIMode = "shared"
