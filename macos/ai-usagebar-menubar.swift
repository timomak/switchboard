...
let SWITCHBOARD_PREVIEW = Bundle.main.object(forInfoDictionaryKey: "SwitchboardPreview") as? Bool ?? false
...
func resolveBinary(_ name: String) -> String? {
    ...
    // A fork bundle must use its matching backend, not an older global install.
...
// these are who the Claude and Codex desktop apps are actually signed
...
struct AccountStatus: Equatable {
    ...
    var codexActive: String?
    var codexLabels: [String] = []
    var codexHasLogin = false
    var codexRecoveryRequired = false
    var codexProfileProblems: [String] = []
    var codexError: String?
    var cliHasIdentity = false
    var codexCLIMode = "shared"
    var codexCLIActive: String?
    var codexCLILabels: [String] = []
    var codexCLIHome: String?
    var codexCLIRecovery = false
    var codexCLIProfileProblems: [String] = []
    var codexCLIError: String?
    var connectionTemplates: [ConnectionTemplate] = []
    var cloudActive: String?
    var cloudActiveModel: String?
    var connectionRecovery = false
    var cloudProfiles: [CloudProfile] = []
    var claudeConnectionError: String?
    var claudeConnectionLabel: String?
    var claudeConnectionDesktop = false
    var claudeConnectionCLI = false
    var cloudError: String?
}

struct CloudProfile: Equatable {
    var label: String
    var provider: String
    var model: String?
}
struct ConnectionTemplate: Equatable {
    var id: String
    var name: String
    var fields: [ConnectionField]
}
struct ConnectionField: Equatable {
    var argument: String
    var title: String
    var value: String
}
...
func parseAccountStatus(_ data: Data) -> AccountStatus? {
    ...
    let codexCLI = root["codex_cli"] as? [String: Any]
    ...
    return AccountStatus(desktopAvailable: desktop is [String: Any],
                         ...
                         codexAvailable: codex?["available"] as? Bool ?? false,
                         codexActive: active(codex),
                         codexLabels: labels(codex, "profiles"),
                         codexHasLogin: codex?["has_login"] as? Bool ?? false,
                         codexRecoveryRequired: codex?["recovery_required"] as? Bool ?? false,
                         codexProfileProblems: (codex?["profile_errors"] as? [[String: String]] ?? []).map { "\($0["label"] ?? "Profile"): \($0["error"] ?? "Restore profile metadata.")" },
                         codexError: codex?["error"] as? String,
                         cliHasIdentity: (cli as? [String: Any])?["has_identity"] as? Bool ?? false,
                         codexCLIMode: codexCLI?["mode"] as? String ?? "shared",
                         codexCLIActive: active(codexCLI),
                         codexCLILabels: labels(codexCLI, "profiles"),
                         codexCLIHome: codexCLI?["home"] as? String,
                         codexCLIRecovery: codexCLI?["recovery_required"] as? Bool ?? false,
                         codexCLIProfileProblems: (codexCLI?["profile_errors"] as? [[String: String]] ?? []).map { "\($0["label"] ?? "Profile"): \($0["error"] ?? "Restore profile metadata.")" },
                         codexCLIError: codexCLI?["error"] as? String,
                         connectionTemplates: ((root["codex_cloud"] as? [String: Any])?["templates"] as? [[String: Any]] ?? []).compactMap { row in
                             guard let id=row["id"] as? String, let name=row["name"] as? String else {return nil}
                             let fields=(row["fields"] as? [[String: String]] ?? []).compactMap { f -> ConnectionField? in
                                 guard let argument=f["argument"], let title=f["title"], let value=f["value"] else {return nil}
                                 return ConnectionField(argument: argument, title: title, value: value)
                             }
                             return ConnectionTemplate(id: id, name: name, fields: fields)
                         },
                         cloudActive: ((root["codex_cloud"] as? [String: Any])?["selection"] as? [String: Any])?["active_label"] as? String,
                         cloudActiveModel: ((root["codex_cloud"] as? [String: Any])?["selection"] as? [String: Any])?["active_model"] as? String,
                         connectionRecovery: ((root["codex_cloud"] as? [String: Any])?["selection"] as? [String: Any])?["recovery_required"] as? Bool ?? false,
                         cloudProfiles: ((root["codex_cloud"] as? [String: Any])?["profiles"] as? [[String: Any]] ?? []).compactMap { row in
                             guard let label = row["label"] as? String, let provider = row["provider"] as? String else { return nil }
                             return CloudProfile(label: label, provider: provider, model: row["model"] as? String)
                         },
                         claudeConnectionError: (root["claude_connection"] as? [String: Any])?["error"] as? String,
                         claudeConnectionLabel: (root["claude_connection"] as? [String: Any])?["label"] as? String,
                         claudeConnectionDesktop: (root["claude_connection"] as? [String: Any])?["desktop_selected"] as? Bool ?? false,
                         claudeConnectionCLI: (root["claude_connection"] as? [String: Any])?["cli_selected"] as? Bool ?? false,
                         cloudError: (root["codex_cloud"] as? [String: Any])?["error"] as? String ?? (((root["codex_cloud"] as? [String: Any])?["selection"] as? [String: Any])?["error"] as? String))
}
...
func accountsSummaryLine(_ s: AccountStatus) -> String {
    var parts: [String] = []
    if !s.desktopLabels.isEmpty { parts.append("Claude: \(s.desktopActive ?? "?")") }
    if !s.codexLabels.isEmpty { parts.append("Codex: \(s.codexActive ?? "not saved")") }
    return parts.joined(separator: "   ·   ")
}

/// `-y` because the menu has already asked; without it the binary would prompt
/// on a stdin that is not a terminal and abort.
func codexSwitchArgs(label: String) -> [String] {
    ["codex-account", "switch", label, "--yes"]
}
...
func conflictPreview(_ conflicts: [DeletionConflict], limit: Int = 10) -> String {
    ...
    if conflicts.count > limit {
        lines.append("… and \(conflicts.count - limit)")
    }
...
let LAUNCH_AGENT_LABEL = "io.github.timomak.switchboard-menubar"
...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    let board = SwitchboardModel()
    let boardPopover = NSPopover()
    var boardGeneration = 0
    let usesSwitchboard = true
    ...
    let vendorSubmenuItem = NSMenuItem(title: "Usage display", action: nil, keyEquivalent: "")
    ...
    let codexAccountSubmenu = NSMenu()
    let codexAccountItem = NSMenuItem(title: "Codex Desktop", action: nil, keyEquivalent: "")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        DEF.register(defaults: ["swapShortcutEnabled": true, "compactShortcutEnabled": true, "showBars": false, "boardIconOnly": true, "boardWeekly": true])
    ...
    func installSwapHotKey() {
        guard !SWITCHBOARD_PREVIEW else { return }
    ...
    @objc func handleSwapHotKey() {
        if usesSwitchboard { toggleSwitchboard(); return }
    ...
    @objc func handleCompactHotKey() {
        if usesSwitchboard { DEF.set(!DEF.bool(forKey: "boardIconOnly"), forKey: "boardIconOnly"); return }
    ...
    func buildMenu() {
        ...
        for (item, submenu) in [(desktopAccountItem, desktopAccountSubmenu),
                                (codexAccountItem, codexAccountSubmenu)] {
    ...
    @objc func openPrefs() {
        if prefsWindow == nil {
            ...
            w.title = "Switchboard — Preferences"
    ...
    @objc func settingsChanged() {
        if usesSwitchboard {
            updateBoardIcon(); restartTimer()
            if DEF.bool(forKey: "swapShortcutEnabled") != (swapHotKeyRef != nil) { installSwapHotKey() }
            if DEF.bool(forKey: "compactShortcutEnabled") != (compactHotKeyRef != nil) { installCompactHotKey() }
            return
        }
    ...
    func refresh() {
        if usesSwitchboard { refreshSwitchboard(); return }
    ...
    func renderAccountMenus() {
        ...
        guard let status = lastAccountStatus else {
            ...
            codexAccountItem.isHidden = true
            return
        }
        ...
        fillCodexMenu(status)
        desktopAccountItem.isHidden = !status.desktopAvailable
        codexAccountItem.isHidden = !status.codexAvailable
        desktopAccountItem.isEnabled = !accountSwitchInFlight
        codexAccountItem.isEnabled = !accountSwitchInFlight
        codexAccountItem.title = "Codex Desktop"
    }
    ...
    private func fillCodexMenu(_ status: AccountStatus) {
        codexAccountSubmenu.removeAllItems()
        let ready = !accountSwitchInFlight && !status.codexRecoveryRequired && status.codexError == nil
        if let detail = status.codexError {
            let item = NSMenuItem(title: detail, action: nil, keyEquivalent: "")
            item.isEnabled = false
            codexAccountSubmenu.addItem(item)
        }
        for label in status.codexLabels {
            let item = NSMenuItem(title: label, action: #selector(switchCodexAccount(_:)), keyEquivalent: "")
            item.target = self
            item.representedObject = label
            item.state = label == status.codexActive ? .on : .off
            item.isEnabled = ready && status.codexActive != nil && label != status.codexActive
            codexAccountSubmenu.addItem(item)
        }
        if status.codexRecoveryRequired {
            addAction(codexAccountSubmenu, "Recover interrupted switch…", #selector(recoverCodexAccount), "")
            codexAccountSubmenu.items.last?.isEnabled = !accountSwitchInFlight
            return
        }
        if status.codexHasLogin && status.codexActive == nil {
            let hint = NSMenuItem(title: "Save your current login before switching", action: nil, keyEquivalent: "")
            hint.isEnabled = false
            codexAccountSubmenu.addItem(hint)
        }
        codexAccountSubmenu.addItem(.separator())
        addAction(codexAccountSubmenu, "Save current account…", #selector(saveCodexAccount), "")
        codexAccountSubmenu.items.last?.isEnabled = ready && status.codexHasLogin && status.codexActive == nil
        addAction(codexAccountSubmenu, "Add account…", #selector(addCodexAccount), "")
        codexAccountSubmenu.items.last?.isEnabled = ready
    }

    private func codexLabel(savingCurrent: Bool) -> String? {
        let alert = NSAlert()
        alert.messageText = savingCurrent ? "Save current Codex account" : "Add a Codex account"
        alert.informativeText = savingCurrent
            ? "Choose a profile name for the account currently signed into Codex."
            : "Choose a profile name, then sign in on the official page in your browser. Your running Codex account stays unchanged."
        let field = NSTextField(frame: NSRect(x: 0, y: 0, width: 240, height: 24))
        field.placeholderString = "work"
        alert.accessoryView = field
        alert.addButton(withTitle: savingCurrent ? "Save" : "Sign in")
    ...
    @objc func saveCodexAccount() {
        guard let label = codexLabel(savingCurrent: true) else { return }
        runAccountOperation(args: ["codex-account", "save", label], success: "Current Codex account saved.")
    }

    @objc func addCodexAccount() {
        guard let label = codexLabel(savingCurrent: false) else { return }
        runAccountOperation(args: ["codex-account", "add", label],
                            success: "Codex account saved. Select it under Codex Desktop when you are ready to switch.")
    }

    @objc func switchCodexAccount(_ sender: NSMenuItem) {
        guard let label = sender.representedObject as? String else { return }
        let alert = NSAlert()
        alert.messageText = "Switch Codex Desktop to “\(label)”?"
        alert.informativeText = "Finish active tasks first. Codex will quit and reopen with the selected login. Local chats, projects, and routines stay in the same Codex home."
        alert.addButton(withTitle: "Switch and restart")
        alert.addButton(withTitle: "Cancel")
        NSApp.activate(ignoringOtherApps: true)
        guard alert.runModal() == .alertFirstButtonReturn else { return }
        runAccountOperation(args: codexSwitchArgs(label: label))
    }

    @objc func recoverCodexAccount() {
        let alert = NSAlert()
        alert.messageText = "Recover the interrupted Codex switch?"
        alert.informativeText = "Codex will quit, restore the previous login, and reopen."
        alert.addButton(withTitle: "Recover and restart")
        alert.addButton(withTitle: "Cancel")
        NSApp.activate(ignoringOtherApps: true)
        guard alert.runModal() == .alertFirstButtonReturn else { return }
        runAccountOperation(args: ["codex-account", "recover", "--yes"])
    }
    ...
    private func runAccountSwitch(label: String, desktop: Bool, deleting: [String] = []) {
        runAccountOperation(args: switchArgs(label: label, desktop: desktop, deleting: deleting))
    }

    func runAccountOperation(args: [String], success: String? = nil, successTitle: String = "Account saved") {
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                me.board.busy = false
                me.renderAccountMenus()
                me.fetchAccountStatus()
                if let failure {
                    let alert = NSAlert()
                    alert.alertStyle = .warning
                    alert.messageText = "Account operation failed"
