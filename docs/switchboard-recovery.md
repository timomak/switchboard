...
Ordinary Desktop switches and account capture refuse a pending connection change under the shared account lock, before quitting or touching login backups. The error points to `ai-usagebar cli claude-recover`. Other tools and direct official-app actions do not honor this journal. Finish connection recovery before capturing accounts or using other switching tools. Outside selection edits cause recovery to stop for manual review, never automatic merging. The switcher does not guarantee recovery from disk loss, malicious file edits, or all official-app behavior changes.
